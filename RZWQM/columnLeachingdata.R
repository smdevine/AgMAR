...
dailyPT <- read.table(file.path(workDir, projectName, 'DAILY.PLT'), col.names = c('DAY', 'ACCUMULATED INFILTRATION (CM)', 'WATER FLUX INTO GW (CM/DAY)', 'TOTAL NO3-N IN PROFILE (KG/HA)', 'NO3 FLUX INTO GW (UG/CM^2/DAY)', 'NO3 BREAK THROUGH (MG/L)', 'MINERALIZATION (KG/HA)', 'NITRIFICATION (KG/HA)'), header = FALSE, skip=65)
dailyPT$DAY
...
plotActVsSim <- function(subDir, plot_fname, ylim_upp, columnNames, skiplines, ResidualNO3, fig_label, makeLegend=FALSE) {
  nutri <- read.table(file.path(workDir, subDir, 'nutri.PLT'), col.names = 1:19, header = FALSE, skip=104)
  denitrified <- sum(nutri[12])
  print(paste(denitrified, 'kg ha-1 denitrification'))
  ...
  par(mar=c(3,3.5,0.25,0.25))
  plot(actualNO3$time_daily, actualNO3$gNO3_cm2*10^5, ylim = c(0,ylim_upp), ylab='', xlab='', xlim = c(0,36))
  mtext(expression('NO'[3]*'-N (kg ha'^-1*')'), side = 2, line=2.25)
  mtext('days', side=1, line=2)
  points(x=leachingDF$DAY, y=leachingDF$NO3.N/10, col='red')
  lines(x=c(34.5,34.5,35.5,35.5), y=c(0,rep(21.73, 2), 0), type='s', col='black', lty=1)
  lines(x=c(34.7,34.7,35.3,35.3), y=c(0,rep(ResidualNO3, 2), 0), type='s', col='red', lty=2)
  lines(x=c(36,36,36.8,36.8), y=c(0,rep(denitrified, 2), 0), type='s', col='red', lty=3)
  if(makeLegend) {legend(x='topleft', legend=c(expression('observed NO'[3]*' leached'), expression('simulated NO'[3]*' leached'), expression('observed residual NO'[3]), expression('simulated residual NO'[3]), 'simulated denitrification'), pch=c(1, 1, NA, NA, NA), lty=c(NA, NA, 1, 2, 3), col=c('black', 'red', 'black', 'red', 'red'), bty='n', cex=0.85)}
  text(x=32, y=21, fig_label)
  dev.off()
}
...
plotActVsSim('FineSandyLoam_v19', 'cumulative_NO3_actual_vs_RZWQM_v19.tif', 22, c('DAY', 'ACCUMULATED INFILTRATION (CM)', 'WATER FLUX INTO GW (CM/DAY)', 'TOTAL NO3-N IN PROFILE (KG/HA)', 'NO3 FLUX INTO GW (UG/CM^2/DAY)', 'NO3 BREAK THROUGH (MG/L)', 'MINERALIZATION (KG/HA)', 'NITRIFICATION (KG/HA)'), 65, ResidualNO3 = 21.8, fig_label = 'b')
...
plotActVsSim('FineSandyLoam_v28', 'cumulative_NO3_actual_vs_RZWQM_v28.tif', 22, c('DAY', 'ACCUMULATED INFILTRATION (CM)', 'WATER FLUX INTO GW (CM/DAY)', 'TOTAL NO3-N IN PROFILE (KG/HA)', 'NO3 FLUX INTO GW (UG/CM^2/DAY)', 'NO3 BREAK THROUGH (MG/L)', 'MINERALIZATION (KG/HA)', 'NITRIFICATION (KG/HA)'), 65, ResidualNO3 = 20.54, fig_label = 'c', makeLegend = FALSE)
...
plotActVsSim('FineSandyLoam_v30', 'cumulative_NO3_actual_vs_RZWQM_v30.tif', 22, c('DAY', 'ACCUMULATED INFILTRATION (CM)', 'WATER FLUX INTO GW (CM/DAY)', 'TOTAL NO3-N IN PROFILE (KG/HA)', 'NO3 FLUX INTO GW (UG/CM^2/DAY)', 'NO3 BREAK THROUGH (MG/L)', 'MINERALIZATION (KG/HA)', 'NITRIFICATION (KG/HA)'), 65, ResidualNO3 = 2.44, fig_label = 'a', makeLegend = TRUE)
