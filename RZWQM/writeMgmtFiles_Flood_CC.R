...
cover_crop <- TRUE
rye_date <- '15   10  1992' #day followed by month followed by dummy year
...
rye_line1 <- paste('3', rye_date, '10.0   2 76000.0   0 -99.0   10 -99.0   -99', sep = '  ') #this was ...2 76000.0... for actual cover crop
rye_line2 <- '               3 0.0   0 0.0  1 4 1993'
...
planting_plan <- do.call(c, lapply(years, function(x) {
  if(x%%2==0) {
    ...
    if((x+1)%in%wet_years) {
      ...
      c(corn_line1_final, corn_line2, corn_line3, rye_line1_final, rye_line2_final, rye_line3)
      ...
      if((x+1)%in%wet_years) {
        ...
        rye_line2_final <- rye_line2
        substring(rye_line2_final, 35, 38) <- if(cover_crop){as.character((x+1))}else{as.character(x)}
        c(tomato_line1_final, tomato_line2, tomato_line3, rye_line1_final, rye_line2_final, rye_line3)
